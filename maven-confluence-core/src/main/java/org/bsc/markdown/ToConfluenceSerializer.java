...
public abstract class ToConfluenceSerializer implements Visitor {
    ...
    public static int[] lineAndColFromNode(String text, Node node) {
        ...
        while (pos < length) {
            pos = text.indexOf('\n', pos);
            if (pos == -1) {
                break; // no EOL at the end or we hit the end
            }
    ...
    protected class SpecialPanelProcessor {
        ...
        boolean apply(BlockQuoteNode bqn) {
            ...
            final boolean result = findByClass(bqn.getChildren().get(0),
                                               StrongEmphSuperNode.class,
                                               new FindPredicate<StrongEmphSuperNode>() {

                                                   @Override
                                                   public boolean f(StrongEmphSuperNode p,
                                                                    final Node parent,
                                                                    final int index) {
    ...
    protected <T extends Node, R extends Node> boolean findByClass(T node,
                                                                   final Class<R> clazz,
                                                                   final FindPredicate<R> predicate) {
    ...
    public void visit(VerbatimNode vn) {
        ...
        if (lines.length == 1) {
            _buffer.append("{noformat}").append(vn.getText()).append("{noformat}");
            return;
        }
    ...
    public void visit(CodeNode cn) {
        ...
        if (lines.length == 1) {
            _buffer.append("{{").append(cn.getText()).append("}}");
            return;
        }

        _buffer.append("{code}").append('\n').append(cn.getText()).append("{code}").append('\n');
    }
    ...
    public void visit(final ExpImageNode ein) {
        ...
        try {
            ...
            if (!uri.isAbsolute() && (null == uri.getScheme() || scheme.isEmpty()) && findByClass(ein,
                                                                                                  TextNode.class,
